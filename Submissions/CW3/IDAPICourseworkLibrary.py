...
def readfile(filename):
    ...
    no_variables = int(f.readline())
    # the second line contains the number of root nodes
    no_roots = int(f.readline())
    ...
    no_states = map(int, ((f.readline()).split()))
    # the fourth line contains a single integer the number of data points
    no_data_points = int(f.readline())
    # all the subsequent lines of the file are data points. Each line is extracted as a list of integers which is 
    # appended to the list data_in.
    data_in = []
    for x in range(no_data_points):
        data_in.append(map(int, ((f.readline()).split())))
    f.close()
    return [no_variables, no_roots, no_states, no_data_points, data_in]


# Function to write an array to a results file
# the array is assumed to be either of probabilities of dependencies
def append_array(filename, an_array):
    f = open(filename, 'a')
    for row in range(an_array.shape[0]):
        for col in range(an_array.shape[1]):
            f.write('%6.3f ' % (an_array[row, col]))
        f.write('\n')
    f.write('\n\n')
    f.close()
...
def append_list(filename, a_list):
    f = open(filename, 'a')
    for row in range(a_list.shape[0]):
        f.write('%6.3f ' % (a_list[row]))
    f.write('\n\n')
    f.close()


# Function to write a string to a results file
def append_string(filename, a_string):
    f = open(filename, 'a')
    f.write('%s\n' % (a_string))
    f.close()
